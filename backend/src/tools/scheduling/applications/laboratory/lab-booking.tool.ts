...
export class LabBookingTool extends InstitutionalTool {
    ...
    async execute(
        ...
        if (!resource) {
            throw new BadRequestException(
                `Laboratory not found: ${args.resource}`,
            );
        }

        const date = new Date(
            `${args.date}T00:00:00.000Z`,
        );

        const startTime = new Date(
            `${args.date}T${args.start}:00.000Z`,
        );

        const endTime = new Date(
            `${args.date}T${args.end}:00.000Z`,
        );

        const booking =
            await this.schedulingService.createBooking({
